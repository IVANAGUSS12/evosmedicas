...
def _nombre_pdf_seguro(nombre):
    nombre = _seguro(re.sub(r"\s+", " ", nombre).strip().upper())
...
def _documentacion(page, carpeta):
    ...
    for i in range(total):
        ...
        fila = accion.locator("xpath=ancestor::tr[1]")
        celdas = fila.locator("td")
        tipo_documentacion = ""
        if celdas.count() >= 2:
            tipo_documentacion = celdas.nth(1).inner_text(timeout=5000)
        with page.expect_popup(timeout=120000) as popup_info:
            accion.click()
        popup = popup_info.value
        popup.wait_for_load_state("domcontentloaded")
        ...
        nombre = (
            _nombre_pdf_seguro(tipo_documentacion)
            if tipo_documentacion
            else unquote(Path(urlparse(popup.url).path).name)
...
def _rango_solicitado(trabajo, fecha_ingreso):
    if trabajo["rango_tipo"] == "TODA":
        return _fecha_ingreso_date(fecha_ingreso), datetime.now().date()
    desde = datetime.strptime(trabajo["fecha_desde"], "%Y-%m-%d").date()
    hasta = datetime.strptime(trabajo["fecha_hasta"], "%Y-%m-%d").date()
    if desde > hasta:
        raise RuntimeError("La fecha desde es posterior a la fecha hasta.")
    return desde, hasta


def _rango_indicaciones(trabajo, fecha_ingreso):
    desde, hasta = _rango_solicitado(trabajo, fecha_ingreso)
    return desde - timedelta(days=2), hasta + timedelta(days=2), desde, hasta
...
def _localizar_evento_atencion(page, fecha_evento, trabajo=None):
    ...
    for patron in patrones:
        for selector in selectores:
            encontrados = page.locator(selector).filter(has_text=patron)
            try:
                total = encontrados.count()
            except Exception:
                total = 0
            for indice in range(total):
                candidato = encontrados.nth(indice)
                try:
                    texto = re.sub(
                        r"\s+", " ", candidato.inner_text(timeout=1000)
                    ).strip()
                except Exception:
                    texto = ""
                clave = f"{selector}|{texto}"
                if clave in vistos:
                    continue
                vistos.add(clave)
        ...
        try:
            ...
            pass
    if candidatos and trabajo:
        internacion = str(trabajo.get("internacion") or "").strip()
        if internacion:
            por_internacion = [
                candidato
                for candidato in candidatos
                if internacion in candidato[1]
            ]
            if por_internacion:
                candidatos = por_internacion
        centro = _centro_atencion(trabajo)
        por_centro = [
            candidato
            for candidato in candidatos
            if re.search(rf"\b{re.escape(centro)}\b", candidato[1], re.I)
        ]
        if por_centro:
...
def _abrir_evento_internacion(page, fecha_evento, trabajo=None):
    try:
        evento = _localizar_evento_atencion(page, fecha_evento, trabajo)
    except RuntimeError:
...
def _indicaciones(page, trabajo, fecha_ingreso, carpeta):
    ...
    _abrir_evento_internacion(page, fecha_evento, trabajo)
...
def ejecutar_trabajo(trabajo, usuario, clave, informar):
    ...
    def ejecutar_paso(nombre, accion):
        try:
            return accion()
        except Exception as exc:
            raise RuntimeError(f"Falló durante {nombre}: {exc}") from exc
        ...
        try:
            ...
            if necesita_admision:
                ...
                fecha_ingreso = ejecutar_paso(
                    ...
                    lambda: _buscar_admision(page, trabajo),
                )
            ...
            if necesita_informe:
                ...
                ejecutar_paso(
                    "descarga del informe de hospitalización",
                    lambda: _informe_hospitalizacion(page, carpeta),
                )
                _completar_seccion(carpeta, progreso, "informe")
                informar(
                    "EN PROCESO",
            ...
            if necesita_documentacion:
                ...
                ejecutar_paso(
                    "descarga de documentación adicional",
                    lambda: _documentacion(page, carpeta),
                )
            ...
            if indicaciones_viejo.exists() and not indicaciones.exists():
                indicaciones_viejo.rename(indicaciones)
            ...
            if (
                ...
                ejecutar_paso(
                    "apertura de internación e historia clínica",
                    lambda: _buscar_internacion(page, trabajo),
                )
            ...
            if necesita_indicaciones:
                ind_desde, ind_hasta, solicitado_desde, solicitado_hasta = (
                    _rango_indicaciones(trabajo, fecha_ingreso)
                )
                informar(
                    "EN PROCESO",
                    "Procesando indicaciones y prescripciones "
                    f"({solicitado_desde:%d/%m/%Y} al "
